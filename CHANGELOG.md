...
### Fixed
- **UI works when NORA is mounted under a sub-path behind a reverse proxy** — the UI emitted root-absolute self-links (`/ui/...`, `/api/ui/...`, static assets, inline `fetch`) and redirect targets, so a deployment that mounts NORA under a path prefix (`location /nora/ { proxy_pass http://nora:4000/; }`, which strips `/nora`) served a dashboard whose links resolved without the prefix and 404'd. A response layer now prefixes those links and `Location` headers with the path component of `public_url`. Root-vhost deploys are unaffected — the prefix is empty, so the layer is a no-op (#685).
