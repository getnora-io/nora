...
### Added
- **Cross-repo blob mount (`?mount=&from=`)** — Docker `POST /v2/{name}/blobs/uploads/?mount={digest}&from={repo}` links an existing blob into the target repository via a server-side storage copy (hardlink on local filesystem, CopyObject/rewrite on object stores) and pins the destination by the known digest, answering `201 Created` with `Docker-Upload-UUID` and `Location` per the OCI distribution spec. Anything unmountable — unknown digest, cross-backend, permission denied — degrades to a normal `202 Accepted` upload session so the client falls back to a regular push. Multi-gigabyte base layers shared across repositories are transferred in microseconds instead of re-uploaded (#917).
...
- **CI builds a per-PR test image** — every non-fork PR now pushes `ghcr.io/getnora-io/nora:pr-<number>` (alpine, amd64) after the `test` job passes, and auto-comments the `docker pull` / `docker run` command on the PR so reviewers can test without building locally. Fork PRs are skipped (no `packages:write` token). Relands #907 by @patsevanton (#906, #908).
- **Per-PR test images are now garbage-collected** — a `pr-image-cleanup` workflow deletes `pr-<number>` from GHCR when its PR closes, plus a daily sweep removes any `pr-*` orphan older than 7 days (GHCR has no native tag TTL) (#909, #910).
- **Conan 2.x client compatibility (`/conan/v1/ping`)** — Conan 2.x always sends `GET /conan/v1/ping` as the first request to any remote (hard-coded in `ClientV2Router.ping()`); without it the client fails with `ERROR: b''` and never reaches the working v2 API. The existing v2 `ping` handler (200 + `X-Conan-Server-Capabilities: revisions`) is now also mounted at `/conan/v1/ping` (#901).

### Fixed
- **Retention and GC unified into one scheduler so retention actually runs** — GC and retention ran as two independent tasks with the same default interval (24h), sharing one `cleanup_lock`. Both ticked at the same instant every cycle and the periodic path used `try_lock`, so GC took the lock and retention logged "cleanup lock held, skipping" with no retry. In a deployment with both schedulers on the default 24h interval, **retention never ran at all**. One periodic task now replaces both: each cycle acquires the lock by waiting (not skipping), runs retention first (which creates orphans), then GC (which sweeps them) — one cycle reclaims what the split design needed two. Per-pass intervals are still honored, the boot cycle is preserved, and a panicking pass is caught so it can neither skip later passes nor kill the scheduler (#914).
- **Docker fails closed on transient storage errors in manifest reads** — the namespaced→legacy key fallback fired on ANY storage error, not just `NotFound`. A transient object-store error on the namespaced key silently swapped in the legacy copy and reclassified the manifest as hosted — the same tag GET could answer with different bytes depending on backend weather. Fallback now advances only on `NotFound`; other storage errors return 500 (#911).
- **Atomic token-file writes; store errors answer 503, not 401** — valid API tokens intermittently 401'd under load, then recovered. Token files were rewritten in place with `fs::write` (truncate+write); a concurrent reader could observe a torn/empty file and reject a valid token. Writes now go to a same-directory temp file and rename over the target (atomic on POSIX). Additionally, the middleware collapsed `TokenError::Storage` (I/O or parse failure) into the invalid-credentials 401 and recorded it in the per-IP failure tracker — a storage blip walked clients toward brute-force lockout. Store read failures now answer 503 without recording a failure (#912).
- **npm self-primes metadata on tarball download so `trust_upstream_dates` matures old npm artifacts** — under `trust_upstream_dates`, the quarantine seeds first-seen from the upstream release date so provably-old artifacts mature immediately. But npm metadata was only fetched on packument reads (`npm install` resolves via the packument, then fetches the tarball directly), so a tarball-only request path (mirror pre-fetch, direct URL) never had a date to seed. The tarball download path now self-primes the packument from upstream (same as PyPI/Cargo), and 6 integration tests cover all self-prime paths (#903).
- **OCI-conformant 429 for concurrent-upload limit** — the concurrent-upload ceiling returned a plain-text 429 body. Clients that parse registry errors through `docker/distribution errcode` (podman, skopeo via containers/common) only classify a 429 as retryable when the body is an OCI error JSON with code `TOOMANYREQUESTS`; a plain-text body became `UnexpectedHTTPStatusError` and aborted the push instead of backing off. All three limit-check sites now return the OCI error body plus `Retry-After` (#895).
- **Storage timeouts sized for client-paced streaming** — object_store timeouts are now sized to accommodate slow client-paced streaming instead of timing out mid-transfer (#894).
- **Cancelling a blob upload frees the session instead of leaking it** — `DELETE /v2/{name}/blobs/uploads/{uuid}`, the OCI cancel verb, was never routed: the upload dispatcher matched only `PATCH` and `PUT`, so a client that correctly cancelled got `405 Method Not Allowed` and its session stayed in the map until the 30-minute TTL, still holding one of `max_upload_sessions`. Concurrent CI pushes then filled the ceiling with dead entries and rejected each other with `TOOMANYREQUESTS` while barely any upload was actually in flight — a push that normally takes ~1.5 min stretched past 19 min, nearly all of it re-transferring blobs that were refused at the end. `DELETE` now removes the session and its temp file and answers `204 No Content` (`404` if the session is unknown, `400` on a repository mismatch, matching the `PATCH`/`PUT` name check). Two supporting fixes: a rejected `POST` no longer leaves behind the zero-byte temp file it created before the limit check, and the `429`'s `Retry-After` is jittered over 3–10s instead of a fixed 5s, so refused clients don't re-synchronize onto one cadence and return as a herd. New gauges `nora_upload_sessions` and `nora_upload_in_flight` expose the session-map size and the count of uploads actually streaming, so the gap between them — the idle-session backlog this bug produced — is measurable rather than inferred from client logs (#897).
- **PyPI internal `dates.json` no longer leaks into the simple index** — `ensure_pypi_dates_cached` writes a `pypi/{name}/dates.json` file to storage for `trust_upstream_dates` date-seeding; the `package_versions` listing filtered only `.sha256` sidecars, so `dates.json` appeared as a file entry in the PEP 691 JSON simple index. It has no `.sha256` sidecar, so `hashes` was omitted — `uv` and other strict PEP 691 clients require `hashes`, causing a hard parse failure (`missing field 'hashes'`) on the second `uv sync` (the first succeeds because the upstream HTML index is used, but the second hits the now-cached local listing). The listing now uses `is_valid_pypi_filename` to include only real package files (`.tar.gz`, `.whl`, `.tgz`, `.zip`, `.egg`), excluding any internal bookkeeping files (#891).
...
### Performance
- **Index-rebuild sidecars read concurrently** — `regenerate_repodata` / `regenerate_indexes` read one JSON sidecar per package from storage sequentially — O(repo size) round-trips per upload. On an object-store backend with a ~1.7k-package repo that is ~2 minutes per uploaded file. Shared `read_json_sidecars` helper now fetches with `buffer_unordered(32)` (#913).

### Security
- **h2 0.4.13 → 0.4.17 (RUSTSEC-2026-0258)** — fixes unbounded empty DATA frames advisory (#916).

### Docs
- **Expanded supported registries table in README** — added Pull/Push, Default Upstream, Cache Type and Notes columns; fixed broken legend link (#900).
- **Documented `docker_anon_pull` in env example and llms.txt** — `anonymous_read` does not cover Docker/OCI since #778; the flag was missing from the env example, causing users to set `anonymous_read=true` and wonder why `docker pull` still returns 401 (#899).
