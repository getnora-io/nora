...
:::note
`anonymous_read` управляет всеми реестрами **кроме Docker/OCI**. Анонимный `docker pull` — отдельный переключатель (см. ниже). Поэтому включение `anonymous_read` никогда не открывает доступ к вашим контейнерным образам.
:::

---

## Анонимный docker pull

Анонимный `docker pull` управляется **отдельным** переключателем `docker_anon_pull`, который по умолчанию `false` (fail-closed) и независим от `anonymous_read`:
...
Отдельный переключатель нужен потому, что анонимный pull меняет handshake аутентификации на `/v2/`: при включении пинг `GET /v2/` возвращает `200` (без `WWW-Authenticate`), и клиент Docker идёт без учётных данных. При **выключенном** (по умолчанию) `GET /v2/` возвращает `401` с Basic-челленджем.

При одном лишь `anonymous_read=true` (и выключенном `docker_anon_pull`) исход анонимного `docker pull` зависит от image store клиента: **containerd**-store Docker терпит челлендж на `/v2/` и качает анонимно, а **классический** docker/distribution store кэширует Basic-челлендж и падает с `no basic auth credentials`. `docker_anon_pull=true` делает анонимный pull рабочим единообразно для обоих и прячет его за один явный переключатель.

> **При обновлении:** если вы полагались на `anonymous_read=true` для анонимного `docker pull` (это работало только на containerd image store), включите `docker_anon_pull=true`, чтобы сохранить поведение — `anonymous_read` больше не открывает ни один путь `/v2`.

| Операция Docker | `docker_anon_pull = false` | `docker_anon_pull = true` |
|------------------|--------------------------------|--------------------------------|
| `docker pull` (без логина) | Требуется аутентификация | Не нужна |
| `docker push` | Требуется аутентификация | Требуется аутентификация |
| `GET /v2/_catalog` (список репозиториев) | Требуется аутентификация | Требуется аутентификация |
| Удаление | Требуется аутентификация | Требуется аутентификация |

Примечания:

- **Push не затрагивается.** Запись всегда требует аутентификации; `docker login` + `docker push` работают как раньше.
- **`/v2/_catalog` остаётся под аутентификацией** даже при включённом переключателе — анонимный pull по имени не включает перечисление всех репозиториев.
- **Предъявленные учётные данные проверяются.** Запрос с заголовком `Authorization` аутентифицируется обычным образом (поэтому `docker login -u token -p <nra_…>` продолжает работать, а в аудит-логах виден реальный пользователь); анонимный путь работает только при отсутствии учётных данных.
- **Ограничение клиентов.** Клиенты на базе [`containers/image`](https://github.com/containers/image) (skopeo, podman, buildah) читают параметры аутентификации только из пинга `/v2/`. Когда пинг возвращает `200`, их анонимный pull работает, но аутентифицированные операции могут не работать. `docker`, `oras`, `nerdctl` и containerd не затронуты. Если для семейства `containers/image` нужны и анонимный pull, и аутентифицированные операции — оставьте `docker_anon_pull=false`.
